/**
 * SplashCursor - WebGL Fluid Simulation Effect
...
class SplashCursor {
  ...
  initShaders() {
    ...
    this.baseVertexShader = this.compileShader(gl.VERTEX_SHADER, baseVertexShaderSource);
    ...
    for (const [name, fragmentSource] of Object.entries(shaders)) {
      ...
      const fragmentShader = this.compileShader(gl.FRAGMENT_SHADER, fragmentSource, keywords);
      this.programs[name] = this.createProgram(this.baseVertexShader, fragmentShader);
    }

    // Display material needs special handling for keywords
    this.displayMaterial = {
      program: this.programs.display,
      uniforms: this.getUniforms(this.programs.display),
      setKeywords: (keywords) => {
        const fragmentShader = this.compileShader(gl.FRAGMENT_SHADER, shaders.display, keywords);
        this.displayMaterial.program = this.createProgram(this.baseVertexShader, fragmentShader);
        this.displayMaterial.uniforms = this.getUniforms(this.displayMaterial.program);
      }
    };

    // Setup vertex buffer for rendering
  ...
  initFramebuffers() {
    ...
    this.framebuffers.divergence = this.createFBO(
      simRes.width, simRes.height, r.internalFormat, r.format, texType, this.gl.NEAREST
    );
  ...
  splat(x, y, dx, dy, color) {
    ...
    const splatUniforms = this.getUniforms(this.programs.splat);
    gl.uniform1i(splatUniforms.uTarget, this.framebuffers.velocity.read.attach(0));
    gl.uniform1f(splatUniforms.aspectRatio, this.canvas.width / this.canvas.height);
    gl.uniform2f(splatUniforms.point, x, y);
    gl.uniform3f(splatUniforms.color, dx, dy, 0.0);
    gl.uniform1f(splatUniforms.radius, this.correctRadius(this.config.SPLAT_RADIUS / 100.0));
    this.blit(this.framebuffers.velocity.write);
    this.framebuffers.velocity.swap();

    // Dye splat
    gl.uniform1i(splatUniforms.uTarget, this.framebuffers.dye.read.attach(0));
  ...
  step(dt) {
    ...
    const curlUniforms = this.getUniforms(this.programs.curl);
    gl.uniform2f(curlUniforms.texelSize, this.framebuffers.velocity.texelSizeX, this.framebuffers.velocity.texelSizeY);
    gl.uniform1i(curlUniforms.uVelocity, this.framebuffers.velocity.read.attach(0));
    this.blit(this.framebuffers.curl);

    // Vorticity step
    gl.useProgram(this.programs.vorticity);
    const vorticityUniforms = this.getUniforms(this.programs.vorticity);
    gl.uniform2f(vorticityUniforms.texelSize, this.framebuffers.velocity.texelSizeX, this.framebuffers.velocity.texelSizeY);
    gl.uniform1i(vorticityUniforms.uVelocity, this.framebuffers.velocity.read.attach(0));
    gl.uniform1i(vorticityUniforms.uCurl, this.framebuffers.curl.attach(1));
    gl.uniform1f(vorticityUniforms.curl, this.config.CURL);
    ...
    const divergenceUniforms = this.getUniforms(this.programs.divergence);
    gl.uniform2f(divergenceUniforms.texelSize, this.framebuffers.velocity.texelSizeX, this.framebuffers.velocity.texelSizeY);
    gl.uniform1i(divergenceUniforms.uVelocity, this.framebuffers.velocity.read.attach(0));
    this.blit(this.framebuffers.divergence);

    // Clear pressure
    gl.useProgram(this.programs.clear);
    const clearUniforms = this.getUniforms(this.programs.clear);
    gl.uniform1i(clearUniforms.uTexture, this.framebuffers.pressure.read.attach(0));
    gl.uniform1f(clearUniforms.value, this.config.PRESSURE);
    this.blit(this.framebuffers.pressure.write);
    this.framebuffers.pressure.swap();

    // Pressure iterations
    gl.useProgram(this.programs.pressure);
    const pressureUniforms = this.getUniforms(this.programs.pressure);
    gl.uniform2f(pressureUniforms.texelSize, this.framebuffers.velocity.texelSizeX, this.framebuffers.velocity.texelSizeY);
    gl.uniform1i(pressureUniforms.uDivergence, this.framebuffers.divergence.attach(0));
    for (let i = 0; i < this.config.PRESSURE_ITERATIONS; i++) {
      gl.uniform1i(pressureUniforms.uPressure, this.framebuffers.pressure.read.attach(1));
      this.blit(this.framebuffers.pressure.write);
      this.framebuffers.pressure.swap();
    }
    ...
    const gradientUniforms = this.getUniforms(this.programs.gradientSubtract);
    gl.uniform2f(gradientUniforms.texelSize, this.framebuffers.velocity.texelSizeX, this.framebuffers.velocity.texelSizeY);
    gl.uniform1i(gradientUniforms.uPressure, this.framebuffers.pressure.read.attach(0));
    gl.uniform1i(gradientUniforms.uVelocity, this.framebuffers.velocity.read.attach(1));
    this.blit(this.framebuffers.velocity.write);
    this.framebuffers.velocity.swap();

    // Advection
    gl.useProgram(this.programs.advection);
    const advectionUniforms = this.getUniforms(this.programs.advection);
    gl.uniform2f(advectionUniforms.texelSize, this.framebuffers.velocity.texelSizeX, this.framebuffers.velocity.texelSizeY);
    if (!this.ext.supportLinearFiltering) {
      gl.uniform2f(advectionUniforms.dyeTexelSize, this.framebuffers.velocity.texelSizeX, this.framebuffers.velocity.texelSizeY);
    }
    ...
    gl.uniform1f(advectionUniforms.dissipation, this.config.VELOCITY_DISSIPATION);
    this.blit(this.framebuffers.velocity.write);
    this.framebuffers.velocity.swap();

    if (!this.ext.supportLinearFiltering) {
      gl.uniform2f(advectionUniforms.dyeTexelSize, this.framebuffers.dye.texelSizeX, this.framebuffers.dye.texelSizeY);
    }
    gl.uniform1i(advectionUniforms.uVelocity, this.framebuffers.velocity.read.attach(0));
    gl.uniform1i(advectionUniforms.uSource, this.framebuffers.dye.read.attach(1));
    gl.uniform1f(advectionUniforms.dissipation, this.config.DENSITY_DISSIPATION);
    this.blit(this.framebuffers.dye.write);
    this.framebuffers.dye.swap();
  }
  ...
  drawDisplay(target) {
    ...
    gl.useProgram(this.displayMaterial.program);
