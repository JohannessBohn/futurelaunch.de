...
if (json_last_error() !== JSON_ERROR_NONE) {
    error_log('Invalid JSON received for mark-as-read: ' . $raw_data);
...
// Database connection and mark as read
...
try {
    ...
    $stmt = $db->prepare('UPDATE submissions SET read_status = 1 WHERE id = :id');
    ...
    if ($stmt->execute()) {
         ...
         if ($db->changes() > 0) {
            ...
            sendJsonResponse(false, 'Message with specified ID not found or already marked as read.', null, 404);
         }
    } else {
         error_log('SQLite update failed for ID ' . $messageId . ': ' . $db->lastErrorMsg());
         sendJsonResponse(false, 'Error marking message as read in database.', null, 500);
    }

    $db->close();

} catch (Exception $e) {
    error_log("Database error marking message as read: " . $e->getMessage());
    sendJsonResponse(false, 'An unexpected error occurred while marking as read.', null, 500);
}
?> 